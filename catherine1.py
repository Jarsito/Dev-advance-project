...
dialog_timer = 0
dialog_duration = 2000  # Duración de cada línea de diálogo en milisegundos
dialog_showing = True
...
while True:
    ...
    if dialog_showing:
        ...
        if not dialog_paused:  # Solo moverse si no está en pausa
            ...
            # Mover enemigos y verificar colisiones con Vincent
            for enemy in enemies[:]:
                enemy.move()
                if enemy.rect.bottom < 0:
                    game_over = True  # Fin del juego si un enemigo llega al tope
    
                # Verificar si el enemigo colisiona con Vincent o Vincent_victory
    ...
    if dialog_showing:
        # Mostrar texto del diálogo en pantalla centrado
        font = pygame.font.SysFont(None, 24)
        for i in range(current_dialog_index):
            draw_centered_text(screen, dialog_texts[stage][i], font, HEIGHT // 2 + i * 30)
    else:
